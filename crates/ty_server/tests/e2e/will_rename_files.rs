...
use lsp_types::{FileRename, Uri, WorkspaceEdit};
...
#[test]
fn unsupported_or_cross_workspace_batches_are_atomic() -> anyhow::Result<()> {
    ...
    assert!(rename_edit(&mut server, &[("repo/a/old.py", "repo/b/new.py")]).is_none());
    assert!(rename_edit(&mut server, &[("repo/a/old.py", "repo/a/new.pyi")]).is_none());
    Ok(())
}
