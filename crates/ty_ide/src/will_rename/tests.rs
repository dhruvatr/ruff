...
fn aliases_and_declarations_remain_stable() {
    ...
    assert_file_decline(&db, "/pkg/old.py", "/pkg/new.py", UNSUPPORTED_SEMANTIC);
}
...
fn regular_package_and_runtime_provenance() {
    ...
    assert_file_decline(&shadowed, "/p/old.pyi", "/p/new.pyi", INVALID_RENAME);
    assert_success(
        &shadowed,
...
#[test]
fn unsupported_semantics_decline_atomically() {
    for (name, source) in [
        ...
        let rename = if !source.contains("pkg") && !source.contains("facade") {
            file("/old.py", "/new.py")
        } else {
            file("/pkg/old.py", "/pkg/new.py")
        };
        let reason = match name {
            "R2-03" => UNREPRESENTABLE_IMPORT,
            _ => UNSUPPORTED_SEMANTIC,
        };
        assert_decline(name, &db, &[rename], reason);
    }
}

#[test]
fn unsupported_requests_and_imports_decline() {
    let mut db = test_db(&[
        ("/a/__init__.py", ""),
        ("/a/old.py", ""),
        ("/b/__init__.py", ""),
    ...
    assert_file_decline(&db, "/a/old.py", "/b/new.py", UNREPRESENTABLE_IMPORT);
    db.write_file("/use.py", "import a.old\n").unwrap();
    assert_file_decline(&db, "/a/old.py", "/b/new.py", UNREPRESENTABLE_IMPORT);
    db.write_file("/use.py", "import a.old.missing\n").unwrap();
    assert_file_decline(&db, "/a/old.py", "/a/new.py", UNREPRESENTABLE_IMPORT);
    assert_file_decline(&db, "/a/__init__.py", "/a/new.py", INVALID_RENAME);
    assert_decline(
...
#[test]
fn remaining_atomic_request_contract() {
    let mut db = test_db(&[
        ("/a/__init__.py", ""),
        ("/a/x.py", ""),
        ("/a/o/__init__.py", ""),
    ...
    assert_file_decline(&db, "/a/o/old.py", "/a/new.py", UNSUPPORTED_SEMANTIC);
    let conflicts = || vec![file("/a/x.py", "/a/y.py"), file("/b/x.py", "/b/z.py")];
    assert_decline("qualified use", &db, &conflicts(), UNSUPPORTED_SEMANTIC);
    db.write_file("/u.py", "if q:from a import x\nelse: from b import x\nx")
        .unwrap();
    for (name, rename) in [
        (
            "namespace package",
            vec![PathRename::directory("/ns".into(), "/newns".into())],
        ),
        (
            "relative rebasing",
            vec![file("/a/one/old.py", "/b/new.py")],
        ),
        ("R21-01 conflicting module bindings", conflicts()),
        ("extension change", vec![file("/x.py", "/x.pyi")]),
        (
            "separate runtime and stub pair",
            vec![
                file("/pkg/old.py", "/pkg/new.py"),
        ...
        assert!(
            will_rename_paths(&db, &rename, &db.project().files(&db), |_| true).is_err(),
            "{name}"
        );
    }
}
...
fn assert_file_decline(db: &TestDb, old: &str, new: &str, reason: &'static str) {
    assert_decline(old, db, &[file(old, new)], reason);
}

fn assert_success(db: &TestDb, renames: &[PathRename], expected: &[(&str, &str)]) {
    let edits = will_rename_paths(db, renames, &db.project().files(db), |_| true).unwrap();
...
fn assert_decline(name: &str, db: &TestDb, renames: &[PathRename], reason: &'static str) {
    assert_eq!(
