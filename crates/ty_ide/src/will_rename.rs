...
//! sources. It does not rename anything itself. Instead, it either returns every safe edit or
//! declines the entire batch. This prevents callers from applying a partial edit set that leaves
//! imports inconsistent after the filesystem operation.
//!
//! # Supported renames
//!
...
//! their inferred module is affected and every available source binding supports one spelling
...
//!
//! Unsupported affected syntax declines the complete batch. This includes imports that require
//! splitting or relative rebasing, ambiguous binding policies, affected qualified writes or
//! deletes, `global`- or `nonlocal`-dependent occurrences, and references reached only through a
//! star-import export chain. The path of a direct star import may still be rewritten.
//!
//! Coincidental text is not a reference. The feature does not use `__all__`, dynamic lookup, or
//! runtime strings to discover affected modules. Malformed forward annotations and legacy type
//! comments are ignored. Failures while reading or analyzing a relevant source, and conflicting
//! generated edits, also decline the entire batch.
...
use ty_python_semantic::{
    HasType, ImportAliasResolution, ResolvedDefinition, SemanticModel, SourceBindings,
    definitions_for_imported_symbol,
};
use unicode_normalization::UnicodeNormalization;

/// Computes normalized source edits for one atomic rename batch.
///
/// `files` is the candidate set discovered by the caller. Files outside `in_scope` are skipped.
/// Directly renamed files are added automatically and must be in scope; directory contents are not
/// discovered here and must be included in `files` by the caller.
///
/// Returned edits are sorted, deduplicated, and guaranteed not to overlap.
///
/// # Errors
///
/// Returns a reason, without any partial edit set, when the rename or a relevant candidate cannot
/// be handled safely.
pub fn will_rename_paths(
...
) -> Result<Vec<FileRenameEdit>, &'static str> {
    let plan = RenamePlan::new(db, renames).ok_or(INVALID_RENAME)?;
    ...
    for file in plan.rules.iter().filter_map(RenameRule::file) {
        in_scope(file).then_some(()).ok_or(OUT_OF_SCOPE)?;
        files.push(file);
    }
    ...
    normalize(edits.into_iter().flatten().collect()).ok_or(CONFLICTING_EDITS)
}

/// One filesystem path rename in an atomic batch.
...
const INVALID_RENAME: &str = "unsupported rename request";
...
impl RenamePlan {
    fn new(db: &dyn Db, renames: &[PathRename]) -> Option<Self> {
        ...
        let mut paths: Vec<_> = renames
            .iter()
            .flat_map(|rename| [&rename.old_path, &rename.new_path])
            .map(|path| SystemPath::absolute(path, cwd))
            .collect();
        paths.sort_unstable();
        (!paths
            .windows(2)
            .any(|paths| paths[1].starts_with(&paths[0])))
        .then_some(())?;
        let mut rules: Vec<_> = renames
            .iter()
            .map(|rename| RenameRule::new(db, rename))
            .collect::<Option<_>>()?;
        ...
        (!rules.is_empty()
            && !rules
                .windows(2)
                .any(|rules| rules[0].rewrites_name(&rules[1].old_name).is_some()))
        .then_some(())?;
...
fn edits_for_file(
    ...
    let (mut edits, changes) = imports.finish().ok_or(UNREPRESENTABLE_IMPORT)?;
    ...
    edits.extend(semantics.finish().ok_or(UNSUPPORTED_SEMANTIC)?);
    Ok(edits
        .into_iter()
...
impl<'db> ImportPass<'_, 'db> {
    fn finish(self) -> Option<(Vec<(TextRange, String)>, Changes<'db>)> {
        self.valid.then_some((self.edits, self.changes))
    }

    fn record(&mut self, alias: &ast::Alias, old: &str, new: &str) {
    ...
    fn import(&mut self, import: &ast::StmtImport) -> Result<(), ()> {
        for alias in &import.names {
            let written = ModuleName::new(alias.name.as_str()).ok_or(())?;
            ...
            self.record(alias, old.first_component(), new.first_component());
    ...
    fn import_from(&mut self, import: &ast::StmtImportFrom) -> Result<(), ()> {
        ...
        let written_parent = match self.moved_source.filter(|_| import.level > 0) {
            Some(source) => source
                .relative_parent(import.level, &old_parent)
                .ok_or(())?,
        ...
        for alias in &import.names {
            let (module, resolved) =
                imported_symbol(self.db, self.model, import, alias, self.plan)?;
            unresolved |= !resolved;
            let parent = if let Some(module) = module {
                ...
                if let Some((_, new)) = self.plan.rewrite(self.db, module) {
                    ...
                    self.record(alias, old.last_component(), new.last_component());
        ...
        if desired_parent != written_parent {
            let module = import.module.as_ref().ok_or(())?;
            let replacement = if import.level == 0 {
                desired_parent.as_str().to_string()
            } else {
                relative_replacement(module.as_str(), &written_parent, &desired_parent).ok_or(())?
            };
...
impl SemanticPass<'_, '_> {
    fn finish(self) -> Option<Vec<(TextRange, String)>> {
        self.valid.then_some(self.edits)
    }

    fn name(&mut self, name: &ast::ExprName) {
        if name.ctx.is_del() {
            self.valid &= !self.plan.terminal(name.id.as_str());
            return;
        }
    ...
    fn attribute(&mut self, attribute: &ast::ExprAttribute) -> TraversalSignal {
        ...
        let decision = match bindings.as_ref() {
            Some(bindings) => self.binding_decision(bindings, rewrite.is_some(), |definition| {
                let Type::ModuleLiteral(module) = self.model.source_binding_type(definition) else {
                    return Err(());
                };
                Ok(self
                    .plan
            ...
            None => rewrite.as_ref().map_or(Decision::Keep, |(_, name)| {
        ...
        let unresolved_file = bindings.is_none()
            && module_from_type(self.model, &*attribute.value)
                .and_then(|module| {
    ...
    fn apply(&mut self, range: TextRange, decision: Decision, reject_change: bool) {
        match decision {
            ...
            Decision::Unsupported => self.valid = false,
        }
    }

    fn declaration_is_affected(&self, node: AnyNodeRef<'_>) -> bool {
        let names = match node {
            AnyNodeRef::StmtGlobal(global) => &global.names,
            AnyNodeRef::StmtNonlocal(nonlocal) => &nonlocal.names,
            _ => return false,
        };
        names.iter().any(|name| self.plan.terminal(name.as_str()))
    }
}

impl<'a> SourceOrderVisitor<'a> for SemanticPass<'a, '_> {
    fn enter_node(&mut self, node: AnyNodeRef<'a>) -> TraversalSignal {
        if self.declaration_is_affected(node) {
            self.valid = false;
            return TraversalSignal::Skip;
        }
