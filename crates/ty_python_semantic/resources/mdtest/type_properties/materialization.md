...
An ordinary protocol variable is not a class variable, so a concrete class satisfying
`type[InstanceOnlyAny]` does not need to define it on the class object. Attribute lookup on
`type[InstanceOnlyAny]` intentionally exposes the member anyway, and that broad lookup is
materialized:
...
class InstanceOnlyImplementation:
    def __init__(self) -> None:
        self.value = 1

instance_only_class: type[Top[InstanceOnlyAny]] = InstanceOnlyImplementation
