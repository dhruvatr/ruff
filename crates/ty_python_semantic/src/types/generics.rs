...
impl<'db, 'c> SpecializationBuilder<'db, 'c> {
    ...
    pub(crate) fn build_inference_with(
        ...
        let declared = self.constraint_set_types(self.declared_pending, true);
        let inferred = self.constraint_set_types(Some(self.inferred_pending), false);
        let db = self.db;
        let constraints = self.constraints;
        // Prefer the inferred candidate unless it is mutually assignable with the declared
        // candidate. A preference is only applied when it satisfies the already-conjoined path;
        // otherwise the default solver chooses another valid solution.
        let mut choose_valid = |typevar: BoundTypeVarInstance<'db>,
            ...
            let candidate = match (inferred, declared) {
                (Some(inferred), Some(declared))
                    if inferred.is_assignable_to(db, declared)
                        && declared.is_assignable_to(db, inferred) =>
                {
                    Some(declared)
                }
    ...
    fn constraint_set_types(
        ...
        let mut variances: FxHashMap<BoundTypeVarIdentity<'db>, TypeVarVariance> =
            FxHashMap::default();
        let solutions = constraints.solutions_with(
            ...
            |variance, bounds| {
                variances
                    .entry(bounds.bound_typevar.identity(self.db))
                    .and_modify(|current| *current = current.join(variance))
                    .or_insert(variance);
                PathBounds::default_solve(self.db, self.constraints, bounds)
            },
        );
